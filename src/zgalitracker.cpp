...
CzJIYOTracker::CzJIYOTracker(std::string strWalletFile)
{
...
CzJIYOTracker::~CzJIYOTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzJIYOTracker::Init()
{
...
bool CzJIYOTracker::Archive(CMintMeta& meta)
{
...
bool CzJIYOTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzJIYOTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzJIYOTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzJIYOTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzJIYOTracker::GetSerialHashes()
{
...
CAmount CzJIYOTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzJIYOTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzJIYOTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzJIYOTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzJIYOTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzJIYOTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzJIYOTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzJIYOTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzJIYOTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzJIYOTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzJIYOTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzJIYOWallet* zJIYOWallet)
{
    bool iszJIYOWalletInitialized = (NULL != zJIYOWallet);
    CMintMeta meta;
    ...
    if (! iszJIYOWalletInitialized)
        zJIYOWallet = new CzJIYOWallet(strWalletFile);
    meta.isSeedCorrect = zJIYOWallet->CheckSeed(dMint);
    if (! iszJIYOWalletInitialized)
        delete zJIYOWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzJIYOTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzJIYOTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzJIYOTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzJIYOTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzJIYOTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzJIYOTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzJIYOWallet* zJIYOWallet = new CzJIYOWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zJIYOWallet);
        }
        delete zJIYOWallet;
        LogPrint("zero", "%s: added %d dzjiyo from DB\n", __func__, listDeterministicDB.size());
    }
