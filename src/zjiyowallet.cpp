...
CzJIYOWallet::CzJIYOWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zjiyo seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZJIYOSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZJIYOSeed_deprecated()) {
                    LogPrintf("%s: Updated zJIYO seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zjiyo seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zjiyo wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzJIYOWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZJIYOCount(nCountLastUsed);
    else if (!walletdb.ReadZJIYOCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzJIYOWallet::Lock()
{
    seedMaster = 0;
}

void CzJIYOWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzJIYOWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZJIYO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzJIYOWallet::LoadMintPoolFromDB()
{
...
void CzJIYOWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzJIYOWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzJIYOWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zjiyoTracker->HasPubcoinHash(pMint.first)) {
...
bool CzJIYOWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZJIYO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zjiyoTracker which also adds to database
    pwalletMain->zjiyoTracker->Add(dMint, true);
...
void CzJIYOWallet::SeedToZJIYO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzJIYOWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzJIYOWallet::GenerateDeterministicZJIYO(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzJIYOWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzJIYOWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzJIYOWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
