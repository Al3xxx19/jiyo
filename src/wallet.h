...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT15000IFMN = 3,
    ONLY_NONDENOMINATED_NOT15000IFMN = 4, // ONLY_NONDENOMINATED and not 15000 JIYO at the same time
...
enum ZerocoinSpendStatus {
    ...
    ZJIYO_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZJIYO_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZJIYOOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzjiyoAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzJIYOTracker> zjiyoTracker;
    ...
    void setZWallet(CzJIYOWallet* zwallet)
    {
        zwalletMain = zwallet;
        zjiyoTracker = std::unique_ptr<CzJIYOTracker>(new CzJIYOTracker(strWalletFile));
    }

    CzJIYOWallet* getZWallet() { return zwalletMain; }
    ...
    /** zJIYO reset */
    boost::signals2::signal<void()> NotifyzJIYOReset;
