...
/** Fees smaller than this (in ujiyo) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zJIYO spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zJIYO is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zJIYO spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zJIYO without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zJIYO spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zJIYO spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zjiyo");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zJIYO mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    else if (nHeight <= 2999999 && nHeight >= 2000000) {
        nSubsidy = 3 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZJIYOStake)
{
    int64_t ret = 0;
	...
	else if (nHeight <= 29999 && nHeight >= 10000) {
        ret = blockValue * 0.90;
    }
	else if (nHeight <= 59999 && nHeight >= 30000) {
        ret = blockValue * 0.85;
    }
	else if (nHeight <= 99999 && nHeight >= 60000) {
        ret = blockValue * 0.8;
    }
	else if (nHeight <= 199999 && nHeight >= 100000) {
        ret = blockValue * 0.75;
    }
    else if (nHeight <= 399999 && nHeight >= 200000) {
        ret = blockValue * 0.7;
    }
    else if (nHeight <= 899999 && nHeight >= 400000) {
        ret = blockValue * 0.65;
    }
    else if (nHeight <= 1399999 && nHeight >= 900000) {
        ret = blockValue * 0.6;
    }
    else if (nHeight <= 1999999 && nHeight >= 1400000) {
        ret = blockValue * 0.55;
    }
    else {
		ret = blockValue * 0.50;
    ...
    // when zJIYO is staked, better payouts.
    if (isZJIYOStake) {
...
bool UpdateZJIYOSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    BlockToZerocoinMintList(block, listMints);
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    //Track zJIYO money supply in the block index
    if (!UpdateZJIYOSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zJIYO supply for block=%s height=%d", __func__,
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zJIYO stake block is less than allowed start height", __func__);

    if (CZJiyoStake* zJIYO = dynamic_cast<CZJiyoStake*>(stake)) {
        CBlockIndex* pindexFrom = zJIYO->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zJIYO stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zJIYO stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum = ParseChecksum(nCheckpoint, libzerocoin::AmountToZerocoinDenomination(zJIYO->GetValue()));
        if (nChecksum != zJIYO->GetChecksum())
            return error("%s: accumulator checksum is different than the block %d blocks previous. stake=%d block240=%d", __func__, Params().Zerocoin_RequiredStakeDepth(), zJIYO->GetChecksum(), nChecksum);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZJIYO() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zJIYO fails context checks", __func__));
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> jiyoInputs;
        std::vector<CTxIn> zJIYOInputs;

        for (CTxIn stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                zJIYOInputs.push_back(stakeIn);
            }else{
                jiyoInputs.push_back(stakeIn);
            }
        }
        const bool hasJIYOInputs = !jiyoInputs.empty();
        const bool hasZJIYOInputs = !zJIYOInputs.empty();
        ...
        for (CTransaction tx : block.vtx) {
            for (CTxIn in: tx.vin) {
                ...
                if(hasJIYOInputs)
                    ...
                    for (CTxIn jiyoIn : jiyoInputs){
                        if(jiyoIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zJIYO inputs.
            if(hasZJIYOInputs){

                for (CTxIn zJiyoInput : zJIYOInputs) {
                    CoinSpend spend = TxInToZerocoinSpend(zJiyoInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                        return state.DoS(100,error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zjiyo");
