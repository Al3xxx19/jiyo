...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZJiyoStake(spend));
    } else {
        ...
        CJiyoStake* jiyoInput = new CJiyoStake();
        jiyoInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(jiyoInput);
    }
